...
async function run() {
  ...
  if (argv.length < 5) {
    console.log(
      'Uso: npx tsx scripts/add-user.ts <name> <tec_username> <tec_password> <telegram_chat_id> <drive_root_folder_id>',
    );
    process.exit(1);
  }
  ...
  const storage_provider = drive_root_folder_id ? 'drive' : 'none';

  try {
    await pool.query(
      `INSERT INTO users (name, tec_username, tec_password_enc, telegram_chat_id, drive_root_folder_id, storage_provider, is_active)
      ...
      [name, tec_username, encrypted_pwd, telegram_chat_id, drive_root_folder_id, storage_provider],
    );
